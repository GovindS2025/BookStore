...
const AdminBooksManagementPage = () => {
  ...
  const handleSave = async () => {
    ...
    const parsedPublicationYear = parseInt(currentBook.publicationYear, 10);
    const parsedPrice = parseFloat(currentBook.price);
    const parsedQuantity = parseInt(currentBook.quantity, 10);
    ...
    if (mode === 'add' && !selectedFile) {
      showSnackbar('Please select an image file for a new book.', 'warning');
      return;
    }
    ...
    const bookDataForApi = {
      ...
      publisher: currentBook.publisher,
              authors: Array.isArray(currentBook.authors) 
          ? currentBook.authors 
          : currentBook.authors.split(',').map(a => a.trim()).filter(Boolean),
    };
